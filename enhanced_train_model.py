# enhanced_train_model_updated_full.py
import pandas as pd
import torch
...
def process_dataset_by_language_and_media():
    ...
    for media_value in unique_media_types:
        ...
        # NEW: Memory cleanup after saving embeddings
        try:
            del embeddings
            torch.cuda.empty_cache()
        except Exception:
            # If torch.cuda.empty_cache() exists, call it; otherwise ignore
            pass
        ...
        if 'lang_id' in media_subset.columns:
            ...
            for lang_id in unique_lang_ids:
                ...
                # Attempt to load saved main embeddings to pick language slice if available
                try:
                    main_embeddings = torch.load(f'{media_name}_embeddings.pt', weights_only=True)
                    lang_embeddings = main_embeddings[lang_positions]
                except Exception:
                    # If not available for some reason, recompute (rare)
                    lang_texts = [processor.create_balanced_enhanced_text(row) for _, row in lang_subset.iterrows()]
                    lang_embeddings = processor.model.encode(
                        lang_texts,
                        convert_to_tensor=True,
                        show_progress_bar=True,
                        batch_size=16,
                ...
                if item_count >= min_items_threshold:
                    ...
                    # NEW: Memory cleanup for language-specific embeddings
                    try:
                        del lang_embeddings
                        torch.cuda.empty_cache()
                    except Exception:
