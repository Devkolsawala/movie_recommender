...
#     def create_balanced_enhanced_text(self, row):
#         """Create balanced text representation that doesn't over-emphasize genres"""
#         components = []
        
#         # Title (most important - give it weight but not too much)
#         title = self.clean_text(row['title'])
#         if title:
#             components.extend([title] * 2)  # Reduced from 3 to 2
        
#         # Description (increase weight for semantic content)
#         description = self.clean_text(row['description'])
#         if description:
#             components.extend([description] * 2)  # Increased weight for description
...
#         # Genres (reduced emphasis to prevent genre bias)
#         genres = self.clean_text(row['genres'])
#         if genres:
#             # Add genres but with less emphasis
#             genre_list = [genre.strip() for genre in genres.split(',')]
#             components.extend(genre_list)  # Reduced from * 2 to single occurrence
            
#             # Add more nuanced genre descriptions
#             components.append(f"film with {genres.lower()} elements")
...
#                 components.append("mega blockbuster worldwide phenomenon")
#             elif views > 100000000:  # 100M+
#                 components.append("blockbuster extremely popular film")
#             elif views > 50000000:  # 50M+
#                 components.append("very popular mainstream hit")
#             elif views > 10000000:  # 10M+
#                 components.append("popular well known film")
#             elif views > 1000000:  # 1M+
#                 components.append("moderately popular film")
#             else:
#                 components.append("niche independent film")
        
#         # Media type context
#         media_type_map = {0: 'trailer', 1: 'movie', 2: 'short drama'}
#         if 'ismovie' in row and pd.notna(row['ismovie']):
#             media_type = media_type_map.get(row['ismovie'], 'content')
#             components.append(f"{media_type} entertainment content")
        
#         # Language context (if available)
#         if 'lang_id' in row and pd.notna(row['lang_id']):
#             # Add language context without being too specific
#             components.append(f"language {row['lang_id']} content")
...
#         theme_mapping = {
#             'romance': {
#                 'keywords': ['love', 'romance', 'romantic', 'relationship', 'couple', 'marry', 'wedding', 'heart', 'passion'],
#                 'description': 'romantic emotional storyline'
#             },
#             'action': {
#                 'keywords': ['fight', 'battle', 'war', 'combat', 'violence', 'chase', 'explosion', 'martial', 'warrior'],
#                 'description': 'intense action packed sequences'
#             },
#             'comedy': {
#                 'keywords': ['funny', 'comedy', 'humor', 'laugh', 'comic', 'hilarious', 'amusing', 'joke'],
...
# def process_dataset_by_language_and_media():
#     """Process the dataset with language and media type separation"""
#     # Load full dataset
#     df = pd.read_csv('final_dataset.csv')
...
#     unique_lang_ids = df['lang_id'].unique() if 'lang_id' in df.columns else [None]
#     print(f"🌍 Found {len(unique_lang_ids)} unique languages: {sorted(unique_lang_ids)}")
    
#     print("🔄 Processing dataset with language and media type separation...")
    
#     # Process each media type
#     for media_value, media_name in media_types.items():
#         media_subset = df[df['ismovie'] == media_value].copy()
#         if media_subset.empty:
#             print(f"⚠️ No data found for {media_name}")
#             continue
        
#         print(f"\n📂 Processing {media_name} ({len(media_subset)} items)...")
...
#         # Save the main media type file (all languages combined)
#         media_subset.to_csv(f'{media_name}s.csv', index=False)
#         print(f"💾 Saved {media_name}s.csv with {len(media_subset)} records")
        
#         # Generate embeddings for the full media type (all languages)
#         print(f"🧠 Generating embeddings for all {media_name}s...")
#         embeddings = processor.model.encode(
#             enhanced_texts, 
#             convert_to_tensor=True, 
#             show_progress_bar=True,
...
#         # Save main embeddings file
#         torch.save(embeddings, f'{media_name}_embeddings.pt')
#         print(f"✅ Saved {media_name}_embeddings.pt")
...
#                 print(f"  🌍 Processing {media_name} - Language {lang_id} ({len(lang_subset)} items)")
                
#                 # Get embeddings for this language subset
#                 lang_indices = lang_subset.index - media_subset.index[0]  # Adjust indices
#                 lang_embeddings = embeddings[lang_indices]
                
#                 # Save language-specific files if they have enough data
#                 if len(lang_subset) >= 50:  # Only save if sufficient data
#                     lang_subset.to_csv(f'{media_name}s_lang{lang_id}.csv', index=False)
#                     torch.save(lang_embeddings, f'{media_name}_embeddings_lang{lang_id}.pt')
#                     print(f"  💾 Saved language-specific files for {media_name} - Language {lang_id}")
        
#         # Print sample enhanced text for verification
#         print(f"\n📝 Sample enhanced texts for {media_name}:")
#         for i in range(min(2, len(enhanced_texts))):
#             print(f"Sample {i+1}:")
#             print(f"  Title: {media_subset.iloc[i]['title']}")
#             print(f"  Genres: {media_subset.iloc[i]['genres']}")
#             if 'lang_id' in media_subset.columns:
#                 print(f"  Language: {media_subset.iloc[i]['lang_id']}")
...
#     for media_value, media_name in media_types.items():
#         media_count = len(df[df['ismovie'] == media_value])
#         print(f"{media_name.title()}: {media_count:,} items")
        
#         if 'lang_id' in df.columns:
#             media_by_lang = df[df['ismovie'] == media_value]['lang_id'].value_counts().sort_index()
#             print(f"  Language distribution:")
#             for lang, count in media_by_lang.head(10).items():  # Show top 10 languages
#                 print(f"    Language {lang}: {count:,} items")
#             if len(media_by_lang) > 10:
#                 print(f"    ... and {len(media_by_lang) - 10} more languages")
#         print("-" * 40)
    
#     print("\n✅ All enhanced files saved successfully!")
#     print("\n🔍 Key improvements made:")
#     print("- Added language and media type filtering support")
#     print("- Created separate embeddings for different combinations")
#     print("- Reduced genre over-emphasis in text representation")
#     print("- Increased semantic content weight from descriptions")
#     print("- Added media type and language context to embeddings")
#     print("- Enhanced rating and popularity context")
#     print("- Created both combined and language-specific files")

# def create_language_media_index():
#     """Create an index of available language-media combinations"""
#     try:
#         df = pd.read_csv('final_dataset.csv')
        
#         media_types = {0: 'trailer', 1: 'movie', 2: 'short_drama'}
        
#         print("\n📋 Available Language-Media Combinations:")
#         print("=" * 70)
...
#                 lang_counts = media_subset['lang_id'].value_counts().sort_index()
#                 for lang_id, count in lang_counts.items():
#                     print(f"  Language {lang_id}: {count:,} items")
#             else:
#                 print(f"  Total: {len(media_subset):,} items (no language info)")
        
#         print(f"\n🎯 Usage Examples:")
#         print("# For all movies in any language:")
#         print("recommender = ImprovedMultiGenreRecommender(media_type='movie', lang_id=None)")
#         print("\n# For movies in specific language (e.g., language 1):")
#         print("recommender = ImprovedMultiGenreRecommender(media_type='movie', lang_id=1)")
#         print("\n# For trailers in specific language:")
#         print("recommender = ImprovedMultiGenreRecommender(media_type='trailer', lang_id=1)")
        
#     except FileNotFoundError:
#         print("❌ final_dataset.csv not found. Please ensure the dataset file exists.")
#     except Exception as e:
#         print(f"❌ Error creating index: {e}")
